...
const btnDeleteViPham_click = async (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const response = await del(`/api/v1/violations/${item.maViPham}`)
                if (response.success) {
                    Swal.fire({
                        title: t('configuration.violation.delete_click.success.title'),
                        text: t('configuration.violation.delete_click.success.text'),
                        icon: 'success',
                        timer: 1500,
                    }).then(() => {
                        props.getViPham()
                    ...
                    Swal.fire({
                        title: t('configuration.violation.delete_click.fail.title'),
                        text: t('configuration.violation.delete_click.fail.text'),
                        icon: 'error',
                        timer: 1500,
                    })
                }
    ...
    <div class="table-responsive">
        <table class="table table-hover table-striped table-bordered text-center text-nowrap">
            <thead class="">
                <!-- Tiêu đề bảng -->
                <tr>
                    <th scope="col">{{ $t('configuration.violation.table.no') }}</th>
                    <th scope="col">{{ $t('configuration.violation.table.violation_id') }}</th>
                    <th scope="col">{{ $t('configuration.violation.table.violation_name') }}</th>
                    <th scope="col">{{ $t('configuration.violation.table.amount') }}</th>
                    <th scope="col">{{ $t('configuration.violation.table.tools') }}</th>
                </tr>
            </thead>
            <tbody>
                <tr v-if="listViPham.length === 0" style="text-align: center; font-style: italic">
                    <td colspan="5">{{ $t('configuration.violation.search') }}</td>
                </tr>
                <!-- Dữ liệu bảng -->
